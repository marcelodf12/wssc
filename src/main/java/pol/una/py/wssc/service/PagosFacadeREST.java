...
public class PagosFacadeREST extends AbstractFacade<Pagos> {
    ...
    public void create(Pagos entity) {
        super.create(entity);
    }
    ...
    @Path("{id}")
    @Consumes({"application/xml", "application/json"})
    public void edit(@PathParam("id") Integer id, Pagos entity) {
        super.edit(entity);
    }
    ...
    @Path("{from}/{to}")
    @Produces({"application/xml", "application/json"})
    public List<Pagos> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
